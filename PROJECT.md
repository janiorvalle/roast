...
| 19 | **Reviewer isolation uses first-party harness primitives only** — native flags, native permission profiles, native sandboxes; never OS-level sandboxes, auth staging, managed-policy scanning, or environment reconstruction | The harness vendors own enforcement (and platform support comes free). Proven pattern: quest's dispatcher "guarded" mode. A read-only reviewer must never carry more isolation machinery than the full-write workers that build the code. (Owner decision, 2026-08-02.) |
